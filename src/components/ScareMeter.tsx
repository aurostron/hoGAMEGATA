...
interface ScareMeterProps {
  ...
  scareProfile: ScareProfile | null;
...
export default function ScareMeter({ scareRating, scareProfile, reviewCount }: ScareMeterProps) {
  ...
  const isPending = scareRating === null || scareRating === undefined || !scareProfile;
    ...
    >
      <div className="flex justify-between items-baseline border-b border-white/5 pb-4">
        <h3 className="font-sans text-[11px] uppercase tracking-widest font-bold text-white/50 flex items-center gap-2">
          ...
          Scare Profile
          {isPending ? (
            <span className="bg-red-950/40 text-red-400/90 border border-red-900/40 text-[9px] font-mono font-semibold px-2 py-0.5 rounded uppercase tracking-wider">
              Pending Analysis
            </span>
          ) : (
            Boolean(reviewCount && reviewCount > 0) ? (
        ...
        {isPending ? (
          <span className="font-mono text-xl font-bold text-white/40">
            ? <span className="text-xs text-white/30">/ 100</span>
          </span>
        ) : (
          <span className="font-mono text-xl font-bold text-white">
            {scareRating} <span className="text-xs text-white/50">/ 100</span>
          </span>
        )}
      </div>
      ...
      <div className="space-y-4">
        {dimensions.map(({ key, label }) => {
          const value = isPending ? null : (scareProfile as any)?.[key] ?? 0;
          return (
            <div key={key} className="space-y-2">
              <div className="flex justify-between items-baseline text-xs tracking-wide">
                ...
                <span className={`font-mono text-xs font-medium ${isPending ? 'text-white/30' : 'text-white'}`}>
                  {isPending ? '?' : `${value}%`}
                </span>
              </div>
      ...
      {!isPending && scareProfile?.playerWarnings && scareProfile.playerWarnings.length > 0 && (
