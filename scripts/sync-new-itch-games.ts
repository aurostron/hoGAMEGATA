...
function parseXmlFeed(xmlText: string): FeedItem[] {
  ...
  while ((match = itemRegex.exec(xmlText)) !== null) {
    ...
    if (titleMatch && linkMatch) {
      ...
      // Target individual itch game URLs
...
async function main() {
  ...
  // 1. In-memory diff against existing catalog: 0 Turso reads!
  console.log("📂 Loading local search index to diff...");
  ...
  // 6. Append to public/search-index.json
  console.log("📝 Updating public/search-index.json...");
  for (const g of validNewGames) {
    catalog.push({
      i: g.id,
      t: g.title,
  ...
  // 7. Mirror to project-hgg.github.io if local folder exists
  const projectHggPath = path.join("c:", "Users", "bapum", "Desktop", "Portfolio", "project-hgg.github.io", "docs", "public", "search-index.json");
  if (fs.existsSync(path.dirname(projectHggPath))) {
    try {
      fs.copyFileSync(indexPath, projectHggPath);
      console.log(`✅ Synced updated index to project-hgg.github.io!`);
    } catch {
      // Ignore if not present on CI
    }
  }
