...
const SCRIPT_MAPPING: Record<string, string> = {
  ...
  "add-custom-game": "scripts/add-custom-game.ts",
  ...
  "check-game": "scripts/check-game.ts",
  "ingest-gog": "scripts/ingest-gog.ts",
  "ingest-steam": "scripts/ingest-steam.ts",
  "merge-duplicates": "scripts/merge-duplicates.ts",
};
  ...
  <style>
    ...
    * {
    ...
    .terminal-container {
      ...
      position: sticky;
      top: 6rem;
      box-shadow: 0 8px 32px rgba(0, 0, 0, 0.6), 0 0 15px rgba(255, 255, 255, 0.02);
    }
...
<body>
  ...
  <main class="container">
    ...
    <div class="controls-grid">
      ...
      <div class="card" style="border-left: 3px solid var(--success);">
        <h2><span style="color:var(--success);">📊</span> Database Statistics</h2>
        <p>Query the PostgreSQL database to print game counts, enrichment status, and import source breakdown.</p>
        <div class="actions-row">
          <button class="btn btn-success" onclick="runCommand('check-game', [], 'Fetch Database Statistics')">View Statistics</button>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="actions-row">
          <button class="btn btn-primary" onclick="runIncrementalIngest()">Sync modifications (Incremental)</button>
        ...
        <div class="actions-row" style="margin-top: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
          <input type="checkbox" id="ingestForceUpdate" style="cursor: pointer; width: auto;">
          <label for="ingestForceUpdate" style="font-size: 0.8rem; color: var(--text-muted); cursor: pointer; user-select: none;">
            Force Update Existing Games (updates info & purchase links instead of skipping)
          </label>
        </div>
        <div class="help-tooltip" style="margin-top: 0.75rem;">
          <strong>Import Specific Game from IGDB (Non-interactive):</strong>
        </div>
        <div class="actions-row">
          <div class="form-group">
            <input type="text" id="customGameInput" class="input-control" placeholder="IGDB Slug or ID (e.g. silent-hill-2-2024)">
            <button class="btn btn-cyan" onclick="runCustomGameImport()">Import Game</button>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="border-left: 3px solid #8b5cf6;">
        <h2><span class="num">💽</span> GOG Catalog Ingestion</h2>
        <p>Control the GOG catalog ingestion and price synchronization pipeline.</p>
        <div class="actions-row">
          <button class="btn btn-primary" onclick="runGogIncrementalIngest()">Sync GOG Horror Games (Default 50)</button>
          <button class="btn" onclick="runGogDryRun()">Fast Dry Run (Limit 10)</button>
        </div>
        <div class="actions-row">
          <div class="form-group">
            <label for="gogLimit">Limit Size:</label>
            <input type="number" id="gogLimit" class="input-control" placeholder="e.g. 100" min="1">
            <button class="btn" onclick="runGogIngestWithLimit()">Run Ingestion with Limit</button>
          </div>
        </div>
        <div class="actions-row" style="margin-top: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
          <input type="checkbox" id="gogForceUpdate" style="cursor: pointer; width: auto;">
          <label for="gogForceUpdate" style="font-size: 0.8rem; color: var(--text-muted); cursor: pointer; user-select: none;">
            Force Update Existing Games (updates details & prices)
          </label>
        </div>
      </div>

      <!-- Section Steam Ingest -->
      <div class="card" style="border-left: 3px solid #ff9900;">
        <h2><span class="num">♨️</span> Steam Catalog Ingestion</h2>
        <p>Control the Steam storefront catalog ingestion and price synchronization pipeline.</p>
        <div class="actions-row">
          <button class="btn btn-primary" style="background: linear-gradient(135deg, #ff9900 0%, #d48000 100%); border: none; box-shadow: 0 0 10px rgba(255, 153, 0, 0.4);" onclick="runSteamIncrementalIngest()">Sync Steam Horror Games (Default 50)</button>
          <button class="btn" onclick="runSteamDryRun()">Fast Dry Run (Limit 10)</button>
        </div>
        <div class="actions-row">
          <div class="form-group">
            <label for="steamLimit">Limit Size:</label>
            <input type="number" id="steamLimit" class="input-control" placeholder="e.g. 100" min="1">
            <button class="btn" onclick="runSteamIngestWithLimit()">Run Ingestion with Limit</button>
          </div>
        </div>
        <div class="actions-row" style="margin-top: 0.5rem; display: flex; align-items: center; gap: 0.5rem;">
          <input type="checkbox" id="steamForceUpdate" style="cursor: pointer; width: auto;">
          <label for="steamForceUpdate" style="font-size: 0.8rem; color: var(--text-muted); cursor: pointer; user-select: none;">
            Force Update Existing Games (updates details & prices)
          </label>
        </div>
      </div>
      ...
      <div class="card" style="border-left: 3px solid var(--accent-cyan);">
        ...
        <div class="actions-row" style="display: flex; flex-wrap: wrap; gap: 0.5rem;">
          <button class="btn btn-cyan" onclick="runRetroPreset('all')">Ingest All Collections</button>
          <button class="btn btn-cyan" style="opacity: 0.85;" onclick="runRetroPreset('softwarelibrary_msdos')">MS-DOS Only</button>
          <button class="btn btn-cyan" style="opacity: 0.85;" onclick="runRetroPreset('classicpcgames')">Classic PC Only</button>
          <button class="btn btn-cyan" style="opacity: 0.85;" onclick="runRetroPreset('classicgaming')">Classic Console Only</button>
          <button class="btn btn-cyan" style="opacity: 0.85;" onclick="runRetroPreset('cdromimages')">CD-ROMs Only</button>
        </div>
        <div class="actions-row" style="margin-top: 0.75rem;">
          <div class="form-group" style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
            <label for="retroLimit">Limit Size:</label>
            <input type="number" id="retroLimit" class="input-control" placeholder="Default 200" min="1" style="width: 100px;">
            <label for="retroCollection">Custom Collection:</label>
            <input type="text" id="retroCollection" class="input-control" placeholder="e.g. softwarelibrary_msdos" style="width: 220px;">
            <button class="btn btn-cyan" onclick="runRetroIngestWithOptions()">Run Ingest</button>
          </div>
        </div>
        <div class="help-tooltip" style="margin-top:0.5rem;">
          <strong>Sources:</strong> Archive.org (classicgaming, classicpcgames, softwarelibrary_msdos, cdromimages), Abandonia, MyAbandonware — public domain & abandonware titles only.
        </div>
      </div>
      ...
      <!-- Section 8 -->
      <div class="card">
        <h2><span class="num">8</span> Database Duplicate Management</h2>
        <p>Scan and identify duplicate game entries across the database.</p>
        <div class="actions-row" style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
          <button class="btn btn-primary" onclick="runCommand('merge-duplicates', ['--all', '--dry-run'], 'Dry Run Scan (All)')">Dry Run Scan (All)</button>
          <button class="btn btn-cyan" onclick="runCommand('merge-duplicates', ['--itch', '--dry-run'], 'Dry Run Scan (Itch)')">Dry Run Scan (Itch)</button>
        </div>
        <div class="actions-row" style="margin-top: 0.5rem;">
          <div class="form-group" style="display: flex; align-items: center; gap: 0.5rem; flex-wrap: wrap;">
            <label for="duplicateStore">Store Name:</label>
            <input type="text" id="duplicateStore" class="input-control" placeholder="e.g. Steam, GOG" style="width: 150px;">
            <button class="btn" onclick="runStoreDuplicateScan()">Dry Run Scan (Store)</button>
          </div>
        </div>
        <div class="help-tooltip" style="margin-top:0.5rem; border-left: 2px solid var(--accent-cyan); padding-left: 0.5rem;">
          <strong>Note:</strong> Web dashboard duplicate scanning runs in <strong>Dry Run</strong> mode. To perform interactive merges and select primary/secondary entries, use the main CLI: <code>npm run dev-tools</code> (Option 13).
        </div>
      </div>

      <!-- Section 9 -->
      <div class="card">
        <h2><span class="num">9</span> Early Access Waitlist Manager</h2>
  ...
  <script>
    ...
    function runIncrementalIngest() {
      const forceUpdate = document.getElementById('ingestForceUpdate')?.checked;
      const args = ['--sync'];
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest', args, 'Sync Incremental Ingestion' + (forceUpdate ? ' (Force Update)' : ''));
    }
    ...
    function confirmResetIngest() {
      if (confirm('⚠️ Warning: Are you sure you want to reset all checkpoints and perform a full ingestion from scratch? This will rewrite historical catalogs.')) {
        ...
        const args = ['--reset'];
        if (forceUpdate) args.push('--force-update');
        runCommand('ingest', args, 'Full Reset Ingestion' + (forceUpdate ? ' (Force Update)' : ''));
      }
    }
    ...
    function runIngestWithLimit() {
      ...
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest', args, 'Ingest with Limit ' + limit + (forceUpdate ? ' (Force Update)' : ''));
    }

    function runGogIncrementalIngest() {
      const forceUpdate = document.getElementById('gogForceUpdate')?.checked;
      const args = ['--limit', '50'];
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest-gog', args, 'Sync GOG Horror Games (Default 50)' + (forceUpdate ? ' (Force Update)' : ''));
    }

    function runGogDryRun() {
      runCommand('ingest-gog', ['--limit', '10', '--dry-run'], 'GOG Fast Dry Run (10)');
    }

    function runGogIngestWithLimit() {
      const limit = document.getElementById('gogLimit').value;
      if (!limit || parseInt(limit) <= 0) return alert('Please input a valid limit size.');
      const forceUpdate = document.getElementById('gogForceUpdate')?.checked;
      const args = ['--limit', limit];
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest-gog', args, 'GOG Ingest with Limit ' + limit + (forceUpdate ? ' (Force Update)' : ''));
    }

    function runSteamIncrementalIngest() {
      const forceUpdate = document.getElementById('steamForceUpdate')?.checked;
      const args = ['--limit', '50'];
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest-steam', args, 'Sync Steam Horror Games (Default 50)' + (forceUpdate ? ' (Force Update)' : ''));
    }

    function runSteamDryRun() {
      runCommand('ingest-steam', ['--limit', '10', '--dry-run'], 'Steam Fast Dry Run (10)');
    }

    function runSteamIngestWithLimit() {
      const limit = document.getElementById('steamLimit').value;
      if (!limit || parseInt(limit) <= 0) return alert('Please input a valid limit size.');
      const forceUpdate = document.getElementById('steamForceUpdate')?.checked;
      const args = ['--limit', limit];
      if (forceUpdate) args.push('--force-update');
      runCommand('ingest-steam', args, 'Steam Ingest with Limit ' + limit + (forceUpdate ? ' (Force Update)' : ''));
    }

    function runCustomGameImport() {
      const input = document.getElementById('customGameInput').value.trim();
      if (!input) return alert('Please input a valid IGDB Slug or ID.');
      if (/^\d+$/.test(input)) {
        runCommand('add-custom-game', ['--id', input], 'Import Custom Game (ID: ' + input + ')');
      } else {
        runCommand('add-custom-game', ['--slug', input], 'Import Custom Game (Slug: ' + input + ')');
      }
    }

    function runRetroPreset(collection) {
      const limitVal = document.getElementById('retroLimit').value;
      const limit = (limitVal && parseInt(limitVal) > 0) ? limitVal : '200';
      const args = ['--retro', '--limit', limit];
      if (collection !== 'all') {
        args.push('--collection', collection);
      }
      runCommand('retro-ingest', args, 'Retro Ingest Preset: ' + collection + ' (Limit: ' + limit + ')');
    }

    function runRetroIngestWithOptions() {
      const limitVal = document.getElementById('retroLimit').value;
      const limit = (limitVal && parseInt(limitVal) > 0) ? limitVal : '200';
      const collection = document.getElementById('retroCollection').value.trim();
      const args = ['--retro', '--limit', limit];
      if (collection) {
        args.push('--collection', collection);
      }
      runCommand('retro-ingest', args, 'Retro Ingest (Limit: ' + limit + (collection ? ', Collection: ' + collection : '') + ')');
    }

    function runStoreDuplicateScan() {
      const store = document.getElementById('duplicateStore').value.trim();
      if (!store) return alert('Please input a store name.');
      runCommand('merge-duplicates', ['--store', store, '--dry-run'], 'Dry Run Store Duplicates: ' + store);
    }
